...
impl Iter {
    ...
    fn try_next(&mut self) -> Result<Frame, IterErr> {
        ...
        Ok(match ty {
            ...
            FrameType::PAYMENT => {
                let stream_id = self.bytes.get()?;
                let amount = self.bytes.get_var()?;

                // Read payer length and validate
                let payer_len = self.bytes.get::<u8>()?;
                if payer_len != 32 {
                    return Err(IterErr::Malformed);
                }
                let mut payer = [0u8; PUBLIC_KEY_LENGTH];
                if self.bytes.remaining() < 32 {
                    return Err(IterErr::UnexpectedEnd);
                }
                self.bytes.copy_to_slice(&mut payer);

                // Read token length and validate
                let token_len = self.bytes.get::<u8>()?;
                if token_len != 32 {
                    return Err(IterErr::Malformed);
                }
                ...
                self.bytes.copy_to_slice(&mut token);

                // Read recipient length and validate
                let recipient_len = self.bytes.get::<u8>()?;
                if recipient_len != 32 {
                    return Err(IterErr::Malformed);
                }
                let mut recipient = [0u8; PUBLIC_KEY_LENGTH];
                if self.bytes.remaining() < 32 {
                    return Err(IterErr::UnexpectedEnd);
                }
                self.bytes.copy_to_slice(&mut recipient);
                ...
                if self.bytes.remaining() < SIGNATURE_LENGTH {
                    return Err(IterErr::UnexpectedEnd);
                }
                self.bytes.copy_to_slice(&mut signature);
...
/// PAYMENT frame - enables native payment authorization at the transport layer
#[derive(Clone, PartialEq, Eq)]
pub(crate) struct Payment {
    pub stream_id: StreamId,
    pub amount: u64,
    pub payer: [u8; PUBLIC_KEY_LENGTH],
    pub token: [u8; 32],
...
impl fmt::Debug for Payment {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.debug_struct("Payment")
            ...
            .field("payer", &hex(&self.payer))
            .field("token", &hex(&self.token))
...
impl Payment {
    pub(crate) fn compute_message_hash(&self) -> [u8; 32] {
        let mut hasher = Sha256::new();
        hasher.update(self.stream_id.0.to_le_bytes());
    ...
    /// Encode the payment frame
    pub(crate) fn encode<W: BufMut>(&self, buf: &mut W) {
        buf.write(FrameType::PAYMENT);
        buf.write(self.stream_id);
        buf.write_var(self.amount);
        buf.put_u8(32); // Payer length (fixed 32 bytes)
        buf.put_slice(&self.payer);
        buf.put_u8(32); // Token length (fixed 32 bytes)
        buf.put_slice(&self.token);
        buf.put_u8(32); // Recipient length (fixed 32 bytes)
    ...
    pub(crate) fn size(&self) -> usize {
        1 // frame type
        + VarInt::from_u64(self.stream_id.0).unwrap().size()
        + VarInt::from_u64(self.amount).unwrap().size()
        + 1 + 32 // payer length + payer
        + 1 + 32 // token length + token
...
impl FrameStruct for Payment {
    // Frame type (1) + stream_id (8) + amount (8) + payer_len (1) + payer (32) +
    // token_len (1) + token (32) + recipient_len (1) + recipient (32) +
    // timestamp (8) + nonce (8) + sig_len (8) + signature (64)
    const SIZE_BOUND: usize = 1 + 8 + 8 + 1 + 32 + 1 + 32 + 1 + 32 + 8 + 8 + 8 + SIGNATURE_LENGTH;
}
...
mod test {
    ...
    fn payment_frame_encoding() {
        ...
        let payment = Payment {
            stream_id: StreamId::new(Side::Client, Dir::Bi, 4),
            amount: 10_000,
            payer,
            token: [0u8; 32],
            recipient: [1u8; 32],
        ...
        let frames = frames(buf);
        assert_eq!(frames.len(), 1);
        assert_matches!(&frames[0], Frame::Payment(_));

        if let Frame::Payment(decoded) = &frames[0] {
            assert_eq!(decoded.stream_id, payment.stream_id);
            assert_eq!(decoded.amount, payment.amount);
            assert_eq!(decoded.payer, payment.payer);
            assert_eq!(decoded.token, payment.token);
            assert_eq!(decoded.recipient, payment.recipient);
            assert_eq!(decoded.timestamp, payment.timestamp);
    ...
    fn payment_signature_verification() {
        ...
        let message_hash = payment.compute_message_hash();
        let signature = signing_key.sign(&message_hash);
        payment.signature = signature.to_bytes();

        assert!(payment.verify_signature().is_ok());
    }
    ...
    fn payment_signature_verification_fails_wrong_key() {
        ...
        let payer = wrong_key.verifying_key().to_bytes();
        ...
        assert!(payment.verify_signature().is_err());
    }
    ...
    fn payment_roundtrip() {
        ...
        let mut payment = Payment {
            stream_id: StreamId::new(Side::Client, Dir::Bi, 123),
            amount: 50_000,
            payer,
            token: [5u8; 32],
            recipient: [7u8; 32],
        ...
        if let Frame::Payment(decoded) = &frames[0] {
            ...
            assert_eq!(decoded.timestamp, payment.timestamp);
            assert_eq!(decoded.nonce, payment.nonce);
            assert_eq!(decoded.signature, payment.signature);
            assert!(decoded.verify_signature().is_ok());
        } else {
            panic!("Expected Payment frame");
        }
    }
}
